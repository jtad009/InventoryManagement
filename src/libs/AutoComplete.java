...
public class AutoComplete extends JWindow
        implements KeyListener, FocusListener, ActionListener {

    JTextComponent parent = null;
    private JList lst = null;
    java.util.TreeSet<String> val = new java.util.TreeSet<>();
    java.util.Vector<String> tempVector = new java.util.Vector<>(1, 1);
//declarations for table
    ...
    public AutoComplete(JComponent jc) {
        this();
    ...
    public AutoComplete(JTable t, int col) {
        this(t);
        activeColumn = col;
    }
    ...
    public void keyPressed(KeyEvent ke) {
        ...
        else if((parent instanceof JTextField && kc == KeyEvent.VK_ESCAPE)){
            this.setVisible(false);
    ...
    public void keyReleased(KeyEvent ke) {
        ...
        int kc = ke.getKeyCode();
        if (kc == KeyEvent.VK_F7 || (parent instanceof JTextField && kc == KeyEvent.VK_ENTER)) {
            select(true);
    ...
    /**
     * Fill the auto complete widget with data from the database
     * @param widget the widget to populate
     * @param productResult the database result  set with data
     */
    public void populateFromDB(AutoComplete widget, ResultSet productResult){
        AutoCompleteDbLink acdl = new AutoCompleteDbLink(widget, productResult);
    }
    //connect Widget to the datatabse
    class AutoCompleteDbLink{
        private ResultSet allProducts;
        private AutoComplete ac;
        public AutoCompleteDbLink(AutoComplete acWidget,ResultSet allProductResultSet) {
            ac = acWidget;
            allProducts = allProductResultSet;
            populate();
        }
         private void populate(){
            ...
            try {
                while (allProducts.next()) {
                    String res = allProducts.getString("Name")
                                .concat("- Price: ")
                                .concat(allProducts.getString("Sale Price"))
                                .concat(" - ID/ ")+allProducts.getInt("ID");
                    al.add(res);
                    ac.setItems(al);
